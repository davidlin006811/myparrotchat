...
$(document).ready(function() {
    ...
    // $('.invite-to-group a').on('click', inviteToGrop);
...
function createGroup() {
    ...
    var title = $('<h5>Create A New Group</h5>');
    createGroupDiv.append(title);
    var errorMessage = $('<div class = "error-message"></div>');
    createGroupDiv.append(errorMessage);
    var form = $('<form></form>');
    createGroupDiv.append(form);
    var groupName = $('<div class = "form-group"><label class="col-sm-2 col-form-label"></label><div class = ""col-sm-10""><input type="username" class="form-control" id = "groupName"><div class="help-block with-errors"></div></div></div>');
    form.append(groupName);
    confirmBtn = $('<div class = "confirm-create-group"><button class = "btn btn-success">Confirm</button></div>');
    createGroupDiv.append(confirmBtn);
    cancelBtn = $('<div class = "cancel-create-group"><button class = "btn btn-danger">Cancel</button></div>');
...
function removeMedia(callback) {
    ...
    navigator.getMedia({ "audio": useAudio, "video": useVideo }, function(stream) {
        ...
        $('.video').css('pointer-events', 'auto');
        $('.phone').css('pointer-events', 'auto');
        console.log('remove media successfully');
        var result = true;
        useVideo = false;
